#include "signature_int.h"
#ifdef _FX_SIGNATURE_GMSSL_
#include "signature_Gmssl_dllfunc.h"
...
FX_BOOL  GmsslFunctions::GmsslHandleIsValid()
{
	return m_gmsslHandle != NULL;
}

#if (_FX_OS_ == _FX_WIN32_DESKTOP_ || _FX_OS_ == _FX_WIN64_)
void GmsslFunctions::LoadAllFunctions()
{
	if(m_gmsslHandle)
		return;
	m_gmsslHandle = ::LoadLibraryA("libeay32.dll");
...
#else//(_FX_OS_ == _FX_WIN32_DESKTOP_ || _FX_OS_ == _FX_WIN64_)
#include <stdio.h>
#include <dlfcn.h>
...
void GmsslFunctions::LoadAllFunctions()
{
	...
	m_gmsslHandle = dlopen("libcrypto.1.0.0.dylib", RTLD_NOW);
#elif (_FX_OS_ == _FX_LINUX_DESKTOP_)
	m_gmsslHandle = dlopen("./libcrypto.so.1.0.0", RTLD_NOW);
#endif
    if (NULL == m_gmsslHandle)
    {
        return;
...
#endif //#ifdef _FX_SIGNATURE_GMSSL_